...
**人物参考图是必需输入。** 当前版本只支持“有人物”的卡片流程：必须先提供一张清晰的人物照片或现成人物 IP 图，才能建立人物档案并继续渲染。没有照片时，不进入文章解析和正式出图；如果用户只想做不带人物的通用知识卡片，需要明确说明当前版本尚未提供无人物模式。

## 首次响应

没有已确认人物档案时，不读取文章，只索取以下 4 项：
...
```text
先把人物和视觉系统定下来，再开始拆文章。请准备一张清晰的人物照片或现成 IP 图、人物名称、主题色和卡片动作。人物参考图确认后，我才会读取 Markdown、生成封面和制作内页。
```

## 阶段判断

- 没有人物档案：创建人物草稿。
- 没有照片或现成人物 IP 图：明确告知人物参考图是必需输入，不进入文章解析或渲染。
